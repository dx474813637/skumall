...
<template> 
	<div class="u-p-10 forms-w" >
		<el-form :model="formData" :rules="rules" ref="forms" label-position="top">
...
const { idTypeList, bank_list } = toRefs(finance);
...
function submitForm() {
	forms.value.validate(async (valid) => {
		if (valid) {
			let res = await $api.transfer_random_amount({ ...formData });
