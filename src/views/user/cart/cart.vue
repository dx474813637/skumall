					...
					<div class="cart-box-footer-box u-flex u-flex-between u-p-20">
						...
						<div class="item u-flex">
							...
							<div class="item "> 
								...
								<el-button type="danger" @click="beforeCreateOrderBtn" :disabled="cart_list_checked_num == 0" style="width: 100%;">创建订单</el-button>
							</div>
						</div>
					</div>
				</div>
...
const allCheck = computed({
	get() { 
		return cart_list.value.every(ele => ele.shop.checked) 
	},
	set(v) {  
		handleCheckChange(v, [-1, -1])
	}
})
const sku_ids = computed(() => cart_list.value.map(ele => ele.products.map(item => item.id).join(',')).join(','))
watch(
	() => cart_list.value,
	(val) => {
...
// async function getPidSku(idStr:any) {
// 	if(!idStr) return 
// 	const res = await $api.web_pid_sku1({
// 		params: { id: idStr }
// 	})
// 	if(res.code == 1) {
// 		// let idArr:any = res.list.map((r:any) => r.id); 
// 		cart_list.value.some((cart:any ) => {
// 			return cart.products.some((item:any) => {
// 				let itemIndex = res.list.findIndex((ele:any) => ele.id == item.id)
// 				if(itemIndex != -1) {
// 					let itemObj = res.list[itemIndex]
// 					item = {
// 						...item,
...
function handleNumChange(currentValue: number | undefined, oldValue: number | undefined, positionArr: Array) {
...
function beforeCreateOrderBtn() {
	router.push({name: 'order_create', force: true, state: { message: 'Some Message' }})
}
</script>
